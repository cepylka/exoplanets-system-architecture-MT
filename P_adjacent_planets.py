...
workingTableExoplanets.to_pickle("./newP.pkl") # saving triples of planets with well defined period ratios in a separate .pkl
...
for suchnost in trisuchnosti:
    ...
    g.set(xlabel=f'P$_{{i+1}}$/P$_i$', ylabel=f'P$_{{i+2}}$/P$_{{i+1}}$')
    ...
    ax[0].axvline(x=c,color='g', label=f"Median={format(c,'.4f')}")
    ax[0].axvline(x=e,color='r', label=f"Mean={format(e,'.4f')}")
